...
Сканирование SYN, или так называемое полуоткрытое, или скрытое сканирование. После получения пакета SYN/ACK мы отправляем RST-пакет. 
Это предотвращает повторные попытки сервера выполнить запросы и значительно сокращает время сканирования.
...
TCP FIN отправляет пакет с установленным флагом FIN, который используется для корректного закрытия соединения. 
Следовательно, цель должна ответить RST для closed портов, в соответствии с RFC.
...
Сканирование TCP Xmas ожидает пакеты RST для closed портов в соответствии с RFC.


При UDP сканировании -sU
...
Сканирование UDP менее надежно, поскольку UDP-соединения не имеют статических данных. 
UDP отправляет пакеты, направленные на определенные порты, и надеется, что они дойдут. 
При этом больше внимания уделяется скорости, а не качеству. Отсутствие механизма обратной связи затрудняет 
